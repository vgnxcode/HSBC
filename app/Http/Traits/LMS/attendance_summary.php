...
trait attendance_summary{


    public function latesprocess($employeeid, $monthnumber, $monthyear)
    {
        //$employeeid = '100441';
        //$active_emp = $this->getallactiveemployees();
	$active_emp = DB::connection('mysql6')->table('active_emp')->get();
       $active_emp = json_decode(json_encode($active_emp), true);
       //dd($active_emp);
        foreach ($active_emp as $activekey => $activevalue) {
        ...
        foreach ($getshiftdetails['Shift_Details'] as $key11 => $value11) {
           ...
           if (count($getleaveapp1) > 0) {
            ...
            if (($start_shiftcode == 'VGN_GEN') || ($start_shiftcode == 'VGN_SAP') || ($start_shiftcode == 'VGN_RCP1') || ($start_shiftcode == 'VGN_RCP2') || ($start_shiftcode == 'VGN_MGR') || ($start_shiftcode == 'VGN_DRV')) {
    ...
    public function getdateleavetypeforbackdated($employeeid, $shiftdate)
    {
        $getemp = [];
        $getemp[0] = 1;
        ...
        if(($weekoffholiday['holiday_count'] == 1) || ($weekoffholiday['weekoff_count'] == 1)){
            return $this->getdateleavetypeforbackdated($employeeid, $subday);
        }
        else{
            return $subday;
        }
    ...
    public function getdateleavetypefornextdated($employeeid, $shiftdate)
    {
        $getemp = [];
        $getemp[0] = 1;
        ...
        if(($weekoffholiday['holiday_count'] == 1) || ($weekoffholiday['weekoff_count'] == 1)){
            return $this->getdateleavetypefornextdated($employeeid, $addday);
        }
        else{
            return $addday;
        }
    ...
    public function newattendancesummary($employeeid, $monthnumber, $monthyear){

    $attendance = $this -> latesprocess($employeeid, $monthnumber, $monthyear);
    $laterange1 = 0;
    $laterange2 = 0;
    $laterange3 = 0;
    ...
    foreach($attendance as $key => $value) {
        ...
        if (Carbon::parse($value['shift_date']) -> lt(Carbon::parse($currentdate))) {
            ...
            if (($value['weekoff'] == true) || ($value['holiday'] == true)) {
                ...
                if ($getstartofmonth == $value['shift_date']) {
                    ...
                    foreach($getovernewpunches as $overkey => $overvalue) {
                        if ($overvalue['Date'] != '') {
                            $overarr['punchlist'][] = $overvalue['Date'].
                            ' '.$overvalue['Time'];
                        }
                    ...
                    if (!empty($overarr)) {
                        ...
                        if (($checkweekoff_holidayn['holiday_count'] == 1) || ($checkweekoff_holidayn['weekoff_count'] == 1)) {
                            ...
                            foreach($getovernewpunchesn as $overkey1 => $overvalue1) {
                                if ($overvalue1['Date'] != '') {
                                    $overarrn['punchlist'][] = $overvalue1['Date'].
                                    ' '.$overvalue1['Time'];
                                }
                            ...
                            if ($subtwodayforcheckovertime != '-1') {
                                ...
                                foreach($getvovernewpunches as $eoverkey => $eoverval) {
                                    if ($eoverval['Date'] != '') {
                                        ...
                                        $overarre['punchlist'][] = $eoverval['Date'].
                                        ' '.$eoverval['Time'];
                                    }
                ...
                if (($prev == 0) && ($next == 0)) {
                    ...
                    if (count($getlop) > 0) {
                        ...
                        if (count($checkleave) == 0) {
                            ...
                            $prev_consider_date = $this -> getdateleavetypeforbackdated($employeeid, $value['shift_date'], $value['shift_date']);
                            $next_consider_date = $this -> getdateleavetypefornextdated($employeeid, $value['shift_date'], $value['shift_date']);
                            ...
                            if (($value['shift_code'] == 'VGN_GEN') || ($value['shift_code'] == 'VGN_RCP1') || ($value['shift_code'] == 'VGN_RCP2') || ($value['shift_code'] == 'VGN_MGR') || ($value['shift_code'] == 'VGN_DRV')) {
                    ...
                    if (!empty($value['punchlist'])) {
                        ...
                        if (count($value['lms_applications']) == 1) {
                            ...
                            if ($value['lms_applications'][0] -> partial_days == 'first_half') {
                                ...
                                if (($value['lms_applications'][0] -> type == 'Permission')) {
                                    ...
                                    $add1minuteexception = Carbon::parse($value['shift_date'].
                                        ' '.$explode_to_get_starttime[1]) -> addMinutes(16);
                                } else {
                                    if (($value['lms_applications'][0] -> type == 'LOP')) {
                                        //$attendance[$key]['first_half_lop'] = 0.5;
                                        $attendance[$key]['absent'] = 0.5;
                            ...
                            if ($value['lms_applications'][0] -> partial_days == 'second_half') {
                                ...
                                if (($value['lms_applications'][0] -> type == 'Permission')) {
                                    ...
                                    $explode_to_get_starttime = explode('-', $value['lms_applications'][0] -> start_end_time);
                                    $sub1minuteexception = Carbon::parse($value['shift_date'].
                                        ' '.$explode_to_get_starttime[0]) -> subMinutes(1);
                                        ...
                                        if ($getstartofmonth == $value['shift_date']) {
                                            ...
                                            if (!empty($overarr)) {
                                                ...
                                                foreach($onegetshiftdetailsnew as $keyovershiftcode => $valueovershiftcode) {
                                                    $shiftendtimeforfirstendtime = $valueovershiftcode['Shift_Date'].
                                                    ' '.$valueovershiftcode['End_Time'];
                                                }
                        ...
                        if (count($value['lms_applications']) == 2) {
                            ...
                            if (($firsthalf -> type == 'Permission') && ($secondhalf -> type != 'Permission')) {
                                ...
                                $add1minuteexception_topermission = Carbon::parse($value['shift_date'].
                                    ' '.$explode_to_get_starttime[1]) -> addMinutes(16);
                            ...
                            if (($firsthalf -> type != 'Permission') && ($secondhalf -> type == 'Permission')) {
                                ...
                                $explode_to_get_starttime = explode('-', $firsthalf -> start_end_time);
                                $sub1minuteexception_topermission = Carbon::parse($value['shift_date'].
                                    ' '.$explode_to_get_starttime[0]) -> subMinutes(1);
                                ...
                                if (Carbon::parse($attendance[$key]['first_half_considered_punches'][0]) -> gt($gracetime)) {
                                    ...
                                    if (($value['cadre'] == 'M') || ($value['shift_code'] == 'VGN_MGR')) {
                                        ...
                                        if (($diffinminutes > 960) && ($diffinminutes <= 1860)) {
                                            ...
                                            $attendance[$key]['laterange1'] = $laterange1;
                                            if ($attendance[$key]['latecount'] != 0) {
                                                //Log::alert(($attendance[$key]['laterange1']));
                                                if (($attendance[$key]['laterange1'] % 11) == 0) {
                                                   // Log::alert(($attendance[$key]['laterange1'] % 5));
                                                    $attendance[$key]['first_half_lop'] = 0.5;
                                                }
                                            }
                                        }
    ...
    public function payroll_report_attendance($employeeid, $month, $year, $requestedmonthdate)
    {

        $final_send_array = [];
        $daterange_array = $this->createDateRange(Carbon::parse($requestedmonthdate)->startOfMonth()->toDateString(), Carbon::parse($requestedmonthdate)->endOfMonth()->toDateString(), $format = "Y-m-d");
        $final_send_array['column'] = $daterange_array;
	$final_send_array['column'][count($final_send_array['column'])] = Carbon::parse($requestedmonthdate)->endOfMonth()->toDateString();
	$daterange_array[count($daterange_array)] = Carbon::parse($requestedmonthdate)->endOfMonth()->toDateString();
        ...
        //$active_emps = DB::connection('mysql6')->table('lateacrual_process')->where(['month' => $month, 'year' => $year ])->where('id','>','5435')->where('employeeid','LIKE','1%')->take(80)->get();
	//$active_emps = DB::connection('mysql6')->table('lateacrual_process')->where(['month' => $month, 'year' => $year ])->where('employeeid','LIKE','1%')->take(70)->get();
	//dd($active_emps);
	$active_emps = DB::connection('mysql6')->table('lateacrual_process')->where(['employeeid' => $employeeid,'month' => $month, 'year' => $year ])->get();
        

        foreach ($active_emps as $key => $value) {
                        
                    foreach ($daterange_array as $newkey => $datetocheck) {
                        //$empid_to_process[$value->employeeid][$datetocheck]['lms_attendance'] = [];
                        //$empid_to_process[$value->employeeid][$datetocheck]['late_attendance'] = [];
                        
            //2018-08-01
            
            //$empid_to_process[$datetocheck] = $value;
                $lms_app = DB::connection('mysql6')->table('datetime_ref')->where(['employeeid' => $value->employeeid,'month' => $month, 'year' => $year,'date' => $datetocheck,'final_status' => 'Approved' ])->get();
                $late_deductions = DB::connection('mysql6')->table('latededuction_finaltosap')->where(['employeeid' => $value->employeeid,'month' => $month, 'year' => $year ])->get();
                $leave_balance = DB::connection('mysql6')->table('leave_balance')->where(['employeeid' => $value->employeeid,'month' => $month, 'year' => $year ])->get();

                        //dd($late_deductions);
		
                    
                if (count($lms_app) > 0) {
                    foreach ($lms_app as $nnkey => $nnvalue) {
				
                        $empid_to_process[$value->employeeid][$datetocheck]['lms_attendance'][$nnvalue->partial_days] = $nnvalue->type;
                    }

                }
                
                
                if (count($late_deductions) > 0) {
                    foreach ($late_deductions as $vvkey => $vvvalue) {
                       if((strpos($vvvalue->deduction_stdate_time, $datetocheck) > 0) || (strpos($vvvalue->deduction_etdate_time, $datetocheck) > 0)){
                            $empid_to_process[$value->employeeid][$datetocheck]['late_attendance'][$vvvalue->partial_days] = $vvvalue->type;
                       }
                ...
                if (count($leave_balance) > 0) {
                    foreach ($leave_balance as $balkey => $balvalue) {
                        $empid_to_process[$value->employeeid][$datetocheck]['CL'] = $balvalue->CL;
                        $empid_to_process[$value->employeeid][$datetocheck]['SL'] = $balvalue->SL;
                        $empid_to_process[$value->employeeid][$datetocheck]['PL'] = $balvalue->PL;
                        $empid_to_process[$value->employeeid][$datetocheck]['ML'] = $balvalue->ML;
                        $empid_to_process[$value->employeeid][$datetocheck]['RH'] = $balvalue->RH;
                        $empid_to_process[$value->employeeid][$datetocheck]['Permission'] = $balvalue->Permission;
                        $empid_to_process[$value->employeeid][$datetocheck]['Mispunch'] = $balvalue->Mispunch;
                        $empid_to_process[$value->employeeid][$datetocheck]['Onduty'] = $balvalue->Onduty;
                        $empid_to_process[$value->employeeid][$datetocheck]['Tour'] = $balvalue->Tour;
                        $empid_to_process[$value->employeeid][$datetocheck]['Compoff'] = $balvalue->Compoff;
                        $empid_to_process[$value->employeeid][$datetocheck]['Present'] = $balvalue->Present;
                        $empid_to_process[$value->employeeid][$datetocheck]['LOP'] = $balvalue->LOP;
                        $empid_to_process[$value->employeeid][$datetocheck]['Absent'] = $balvalue->Absent;
                    }
                }
        ...
        $final_send_array['row'] = $empid_to_process;

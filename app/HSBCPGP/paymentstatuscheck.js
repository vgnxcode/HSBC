    ...
    Object.keys(data).forEach(v => {
      if(v >= 25 && v <= 40){
        ...
        xmldata =   `<StatusEnquiry>
                    ...
                    <Key>
                    <ReferenceID>`+data[v].Reference_ID+`</ReferenceID>
                    <MessageID></MessageID>
                    <BatchID></BatchID>
        ...
        const encryptDecryptFunction = async(tosend, msgid) => {
          ...
          //console.log('converting to base64');
            ...
            axios
            .post('https://www.vgn.in/api/hsbccheckstatusforsentdata', {
            _token:"{{csrf_token()}}",datatopass: encodedString,'msgid': msgid
            })
            .then(fres => {
              ...
              if ((response2.statusCode != '')&&(response2.statusCode != 'RJCT')) {
                ...
                const decodedData = atob(response2.responseBase64);
                //console.log(decodedData);
                (async () => {
                  try {
                    const decrypted = await openpgp.decrypt({
                        message: await openpgp.message.readArmored(decodedData),
                    ...
                    xmlDoc = parser.parseFromString(decrypted.data,"text/xml");
                    //console.log(xmlDoc);
                      var utrno = xmlDoc.getElementsByTagName('ClrSysRef')[0].childNodes[0].nodeValue;
                      var statuscode = xmlDoc.getElementsByTagName('TxSts')[0].childNodes[0].nodeValue;
                      var rmsgid = xmlDoc.getElementsByTagName('OrgnlMsgId')[0].childNodes[0].nodeValue;
                      var rpaymentdate = xmlDoc.getElementsByTagName('ReqdExctnDt')[0].childNodes[0].nodeValue;
                      console.log('<---------------------');
                      console.log(utrno);
                      ...
                      axios
                      .post('https://www.vgn.in/api/hsbcfinalupdate', {
                      _token:"{{csrf_token()}}",utrno: utrno,statuscode: statuscode,rmsgid:rmsgid,rpaymentdate: rpaymentdate
                      })
                      .then(finaldata => {

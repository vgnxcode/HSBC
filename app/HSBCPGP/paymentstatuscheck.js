var openpgp = require("openpgp");
const axios = require("axios");
var atob = require("atob");
var btoa = require("btoa");
var DOMParser = require("xmldom").DOMParser;
openpgp.config.ignore_mdc_error = true;
// put keys in backtick (``) to avoid errors caused by spaces or tabs
const pubkey = `-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: BCPG v@RELEASE_NAME@


iyonjeD/xDD1jB+/kf1VxZ7QO6eTb5H3llilWoUAUPe1LTt7zgTg
=Jbf6
-----END PGP PUBLIC KEY BLOCK-----`;

const privkey = `-----BEGIN PGP PRIVATE KEY BLOCK-----


=/DX9
-----END PGP PRIVATE KEY BLOCK-----`; //encrypted private key
const vgnpubkey = `-----BEGIN PGP PUBLIC KEY BLOCK-----


LxCTLyZTxwbxT50hjcu7JvU2cDwpO2Ae+Ron
=pOpx
-----END PGP PUBLIC KEY BLOCK-----`;
const passphrase = `vGn@1942`; //what the privKey is encrypted with

function sapresp() {
    return axios
        .post("https://www.vgn.in/api/gethsbcbankpaymentstatus", {
            _token: "{{csrf_token()}}",
            toprocess: 1,
        })
        .then((res) => {
            //console.log(res.data);
            return res.data;
        })
        .catch((error) => {
            console.error(error);
        });
}

sapresp().then((data) => {
    console.log(data.length);
    if (data.length > 0) {
        //start
        Object.keys(data).forEach((v) => {
            if (v >= 25 && v <= 40) {
                //x >= 0.001 && x <= 0.009
                //console.log(v);
                xmldata =
                    `<StatusEnquiry>
                    <ProfileID>PC000000746</ProfileID>
                    <Key>
                    <ReferenceID>` +
                    data[v].Reference_ID +
                    `</ReferenceID>
                    <MessageID></MessageID>
                    <BatchID></BatchID>
                    <TransactionID></TransactionID>
                    </Key>
                    </StatusEnquiry>`;

                var str_toencrypt = xmldata;

                const encryptDecryptFunction = async (tosend, msgid) => {
                    const privKeyObj = (await openpgp.key.readArmored(privkey))
                        .keys[0];
                    await privKeyObj.decrypt(passphrase);

                    const options = {
                        message: openpgp.message.fromText(str_toencrypt), // input as Message object
                        publicKeys: (await openpgp.key.readArmored(pubkey))
                            .keys, // for encryption
                        privateKeys: [privKeyObj], // for signing (optional)
                    };

                    openpgp
                        .encrypt(options)
                        .then((ciphertext) => {
                            encrypted = ciphertext.data; // '-----BEGIN PGP MESSAGE ... END PGP MESSAGE-----'

                            return encrypted;
                        })
                        .then(async (encrypted) => {
                            //console.log('converting to base64');
                            var encodedString = btoa(encrypted);

                            axios
                                .post(
                                    "https://www.vgn.in/api/hsbccheckstatusforsentdata",
                                    {
                                        _token: "{{csrf_token()}}",
                                        datatopass: encodedString,
                                        msgid: msgid,
                                    },
                                )
                                .then((fres) => {
                                    var response2 = JSON.parse(fres.data);
                                    if (
                                        response2.statusCode != "" &&
                                        response2.statusCode != "RJCT"
                                    ) {
                                        //const privateKey = (await openpgp.key.readArmored([privkey])).keys[0];
                                        //await privateKey.decrypt(passphrase);

                                        const decodedData = atob(
                                            response2.responseBase64,
                                        );
                                        //console.log(decodedData);
                                        (async () => {
                                            try {
                                                const decrypted =
                                                    await openpgp.decrypt({
                                                        message:
                                                            await openpgp.message.readArmored(
                                                                decodedData,
                                                            ),
                                                        privateKeys: [
                                                            privKeyObj,
                                                        ],
                                                    });
                                                console.log(decrypted.data);

                                                var parser, xmlDoc;
                                                parser = new DOMParser();
                                                xmlDoc = parser.parseFromString(
                                                    decrypted.data,
                                                    "text/xml",
                                                );
                                                //console.log(xmlDoc);
                                                var utrno =
                                                    xmlDoc.getElementsByTagName(
                                                        "ClrSysRef",
                                                    )[0].childNodes[0]
                                                        .nodeValue;
                                                var statuscode =
                                                    xmlDoc.getElementsByTagName(
                                                        "TxSts",
                                                    )[0].childNodes[0]
                                                        .nodeValue;
                                                var rmsgid =
                                                    xmlDoc.getElementsByTagName(
                                                        "OrgnlMsgId",
                                                    )[0].childNodes[0]
                                                        .nodeValue;
                                                var rpaymentdate =
                                                    xmlDoc.getElementsByTagName(
                                                        "ReqdExctnDt",
                                                    )[0].childNodes[0]
                                                        .nodeValue;
                                                console.log(
                                                    "<---------------------",
                                                );
                                                console.log(utrno);
                                                console.log(statuscode);
                                                console.log(rmsgid);
                                                console.log(rpaymentdate);
                                                console.log(
                                                    "--------------------->",
                                                );

                                                axios
                                                    .post(
                                                        "https://www.vgn.in/api/hsbcfinalupdate",
                                                        {
                                                            _token: "{{csrf_token()}}",
                                                            utrno: utrno,
                                                            statuscode:
                                                                statuscode,
                                                            rmsgid: rmsgid,
                                                            rpaymentdate:
                                                                rpaymentdate,
                                                        },
                                                    )
                                                    .then((finaldata) => {
                                                        console.log(
                                                            finaldata.data,
                                                        );
                                                        return finaldata.data;
                                                    })
                                                    .catch((error) => {
                                                        console.error(error);
                                                    });
                                            } catch (error) {
                                                console.error(error);
                                            }
                                        })();
                                    }
                                    return fres.data;
                                })
                                .catch((error) => {
                                    console.error(error);
                                });
                            return encrypted;
                        });
                };
                encryptDecryptFunction(str_toencrypt, data[v].Message_Id);
            }
        });
        //end
    } else {
        console.log("No HSBC Payments Status to check...");
    }
});

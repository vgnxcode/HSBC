...
@section('title')
VGN Property Developers |Customer Zone| Occupants Details Update Page
@endsection
    ...
    <div class="col-md-10 col-md-offset-1">
      ...
      <div class="box box-danger">
            <div class="box-header with-border">
                ...
                @if(count($occupantdetails) > 0)<span class="pull-right" ><a href="{{ url('/customerzone/showoccupantdetails') }}" class="btn btn-danger btn-xs"><i class="fa fa-send margin-r-5"></i> Show Occupant Details</a></span>@endif
            </div>
            <!-- /.box-header -->
            <div class="box-body">
...
@endif            
              ...
              @if(count($projects) > 0)
@if($valid != 0)  
                <form id="myForm" method="POST" action="{{ url('/customerzone/occupantdetails') }}" class="form-horizontal" onsubmit="return validate(this);" >
                {{ csrf_field() }}
        ...
        responsive: true
                        ...
                        $("#no_of_occupants").on("change", function(){
                          ...
                          if ((no_of_occupants != '') && (no_of_occupants > 0)) {
                            ...
                            $.post('/api/getcustomerbhk', {_token: "{{ csrf_token() }}",customerid: "{{$getcustomerdata[0]->id}}",Plant_Code:plantcode,Unit_No:unitcode,no_of_occupants:no_of_occupants}, function(data){
                              console.log(data);
                              if (data == 0) {
                                $("#persondetails").hide();
                                $("#no_of_occupants").val('');

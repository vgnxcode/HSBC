@extends('layout.app') 
@section('title') @if(View::exists('title.'.$list->id.'_website'.'_title')) 
@include('title.'.$list->id.'_website'.'_title') @else {{ $list->Project_name }} Landing Page @endif @endsection @section('description') @if(View::exists('metatags.'.$list->id.'_website'.'_metatags')) @include('metatags.'.$list->id.'_website'.'_metatags') @else
...
<meta name="keywords" content="{{ $list->Project_name }} Project vgn, {{ $list->Project_name }} project VGN Projects Estates Pvt Ltd,know more {{ $list->Project_name }} VGN Projects Estates Pvt Ltd, {{ $list->Project_name }} vgn homes " /> @endif @endsection @section('stylesheet')
...
<div class="preloader"></div> <?php $projectname_str = strtolower(str_replace(' ', '_', $list->Project_name)); ?>
<!-- @if(($list->id == '2'))
...
@endif -->
<section style="position:relative;text-align:center;"> @if(in_array($id,config('ProjectSliderList'))) <div class="owl-carousel owl-theme"> @foreach($bannerfiles as $k=>$cons) <div class="item" data-merge="6">
      <picture>
        <source media="(max-width: 768px)" type="image/jpeg" srcset="{{ config('app.AWS_URL')}}/mobile/{{$cons}}">
        <source media="(min-width: 768px)" type="image/jpeg" srcset="{{ config('app.AWS_URL')}}/{{$cons}}">
        <img src="{{ config('app.AWS_URL')}}/{{$cons}}" alt="
																	<?php echo $projectname_str; ?>" title="
																	<?php echo $projectname_str; ?>">
      </picture>
    </div> @endforeach </div> @else <picture>
    ...
    <img src="{{ config('app.AWS_URL')}}/{{$banner}}" alt="
																		<?php echo $projectname_str; ?>" title="
																		<?php echo $projectname_str; ?>">
  </picture> @endif <div class="col-md-3 hide-on-med-and-down" style="position:absolute; top:4%; right:4%;z-index:1">
    <div class="card s12">
      <div class="card-content" style="text-align: center;">
        <span class="card-title" style="color: #EF533B;font-size: 18px;
        font-weight: 400;">For more information Kindly Fill in</span>
        <form class="col s12" action="{{ url('/') }}/{{$projectname_str}}/{{ $newpagename }}" method="post" style="color:#000;">
          {{ csrf_field() }}
          <div class="row">
...
</section> @if($list->single_quote != null) <section class="root-sec " style=" padding: 10px;">
  ...
  <div class="row show-on-med-and-down hide-on-large-only">
    <div class="container" id="enquireformview">
      <div class="row">
        <form class="col s12" action="{{ url('/') }}/{{$projectname_str}}/{{ $newpagename }}" method="post">
          {{ csrf_field() }}
          <div class="row p-0">
            ...
            <div class="input-field col-sm-12 ps-0">
              <input id="Mobile" type="number" name="Mobile" class="validate" pattern="\d*" value="{{ old('Mobile')}}" style="height: 3rem;padding-left: 70px;"> {!! $errors->first('Mobile', ' <span class="errortext">:message</span>') !!} <label for="Mobile">Mobile Number</label>
            </div>
            <div class="input-field col-sm-12 ps-0">
              <input id="Email" type="text" name="Email" class="validate" value="{{ old('Email')}}"> {!! $errors->first('Email', ' <span class="errortext">:message</span>') !!} <label for="Email">Email</label>
            </div>
            <div class="input-field col-sm-12 ps-0">
              <input id="City" type="text" name="City" class="validate" value="{{ old('City')}}"> {!! $errors->first('City', ' <span class="errortext">:message</span>') !!} <label for="City">City</label>
            </div>
...
<!-- <section class="root-sec enquireform ">
 ...
 @endif <section id="indivlist" class="scroll-section root-sec white portfolio-wrap" style="padding-bottom:150px;">
  <nav id="tabnav" style="background-color:#fff !important; color:#000 !important; box-shadow:none !important;">
    <div class="nav-wrapper">
      <ul id="nav-mobile" class="left"> @if(($list->location_section != null) || ($list->googlemap_link != null)) <li>
          ...
          <a href="#specification" data-section="#specification" class="menu-smooth-scroll">Specfications</a>
          ...
          <a href="#floorplan" data-section="#floorplan" class="menu-smooth-scroll">Floor Plans</a>
...
<section id="portfolio" class="scroll-section root-sec white portfolio-wrap" style="margin-top:80px;">
  <div class="container">
    <div class="row">
      <div class="col-sm-12"> @if(($list->location_section != null) || ($list->googlemap_link != null) || ($list->location_adv_section != null)) <div id="location">
        ...
        <!-- <div class="divider"></div> --> @endif @if(View::exists('specification.'.$list->id)) <div id="specification" class="padtop20">
        ...
        <!-- <div class="divider"></div> --> @endif @if(View::exists('amenities.'.$list->id)) <div id="amenities" class="padtop20">
          <div class="container single-post-content">
            <!--<h2 class="styleheader">Amenities</h2>--> @if(View::exists('amenities.'.$list->id)) @include('amenities.'.$list->id) @endif
          </div>
        </div>
        <!-- <div class="divider"></div> --> @endif @if(Storage::disk('s3')->exists('/images/siteplan/' . $list->id . '.jpg')) <div id="siteplan" class="padtop20">
          <div class="container single-post-content">
        ...
        <!-- <div class="divider"></div> --> @endif @if(Storage::disk('s3')->exists('/images/routemap/' . $list->id . '.jpg')) <div id="routemap" class="padtop20">
          <div class="container single-post-content">
            <div class="col-md-6 single-post-page">
        ...
        <!-- <div class="divider"></div> --> @endif @if((count($floorplanfiles) > 0)||(count($consplanfiles) > 0)||(count($ebrochplanfiles) > 0)||$list->youtube_link != null))
...
</section> @endsection @section('footer') @include('footer.index') @endsection @section('scripts') 

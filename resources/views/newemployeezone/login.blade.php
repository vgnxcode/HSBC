...
@section('title')
VGN Projects Estates Pvt Ltd |Employee Zone| Login Page
@endsection

@section('description')
     <META NAME="Subject" CONTENT="Flats and apartments in Chennai">
<meta name="description" content="VGN Employee Login">
...
<meta property='og:url' content='http://vgn.in/employeezone/employeelogin'/>
...
<div class="login-box">
 <a href="{{ url('/')}}">
  ...
  <div class="login-box-body box-body">
    <h4 class="login-box-msg" style="background-color:#f7f7f7; font-size: 18px; text-align: center; padding: 7px 10px; margin-top: 0;">Employee Login</h4>
    <br>  
    ...
    <form id="signinform" method="POST" action="{{ url('/employeezone/employeelogin')}}">
    @csrf
      <div class="form-group has-feedback">
        <span class="glyphicon glyphicon-user form-control-feedback"></span>
        <input type="text" class="form-control" name="username" id="cid"  value="{{ old('username')}}" placeholder="Enter Employee ID (or) Regd. Mail ID (or) Mobile">
...
<form id="forgotform" action="">
      <div class="form-group has-feedback">
        <span class="glyphicon glyphicon-user form-control-feedback"></span>
        <input type="text" class="form-control" id="forgotusername" name="forgotusername" id="fcid"  value="" placeholder="Enter Employee ID (or) Regd. Mail ID (or) Mobile">
  ...
  <div class="row" id="eotpresponsetext" style="margin: 2px;">
    ...
    <a href="#" id="elogin"><i class="fa fa-user margin-r-5"></i> Employee Login</a><br>  
...
<script>
  ...
  var user = $("#cid").val();
    ...
    $("#sendotp").on("click", function(){
      ...
      $("#eotpresponsetext").html('');
      $("#spinner").show();
      if (username != '') {
        $.post("{{ url('/employeezone/forgotpassword')}}",{_token:'{{csrf_token()}}','forgotusername' : username}, function(data){
          $("#spinner").hide();
          console.log(data);
          if (data == '1') {
            $("#sendotp").text('Resend OTP');
            $("#sendotp").attr('disabled',true);
            $("#eotpresponsetext").html(`<div clas="row" id="otpdiv"><p style="margin-top:5px; font-weight:500;">Please enter the OTP Sent to your personal mobile number and Official Mail ID. OTP expires in <span id="otpexpirycounter" style="font-weight:bold;">90</span> Seconds.</p>
              <br>
      ...
      <div class="row">
        ...
        <div class="col-xs-6">
          <button type="button" onclick="esubmitotpfn();" class="btn btn-danger btn-block btn-flat">Submit OTP</button>
        </div>        
      </div>
              </div>`);
            CCOUNT  = '90';
            ...
            alert(data);
            window.location.href="/employeezone/employeelogin";
          }
          //$("#otpsubmit").show();
        });  
      }
  ...
  if ((username1 != '') && (submitotp != '')) {
    $.post("{{ url('/employeezone/otpvalidation')}}",{_token:'{{csrf_token()}}','forgotusername' : username1,'submitotp': submitotp}, function(data){
      $("#spinner").hide();
        if(data == '1'){
            window.location.href='/employeezone/resetlink';
        }
        else{

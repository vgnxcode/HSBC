          ...
          <div class="box box-danger">
            ...
            <tbody>
                ...
                @if(count($getallapplications) > 0)
                                                        ...
                                                        <tr>
                                                        <td>{{$count}}</td>
                                                        @if($appliedvalue['admin_status'] == 'hod_approval')
        ...
        console.log(data);
         ...
         var k;
                ...
                result1+= '<br><input type="hidden" name="subid" id="subid" value="'+data[i]["employeeid"]+'" required><input type="hidden" name="id" id="id" value="'+data[i]["id"]+'" required><div class="form-group" id="rejection_reason"><label for="reason">Rejection Reason: </label><textarea class="form-control" rows="3" name="reason" id="reason" placeholder="Enter rejection reason ..."></textarea></div>';
                 ...
                 if (data[i]['compoff'] != '') {
                 	//console.log(data[i]['compoff'].length);
                  ...
                  for (j = 0; j < data[i]['compoff'][k]['punches_taken'].length; j++) {
                 ...
                 if(data[i]['admin_status'] == 'Pending'){
                //hod only approval
                //result1+= '<br><input type="hidden" name="subid" id="subid" value="'+data[i]["employeeid"]+'" required><input type="hidden" name="id" id="id" value="'+data[i]["id"]+'" required><div class="form-group" id="rejection_reason"><label for="reason">Rejection Reason: </label><textarea class="form-control" rows="3" name="reason" id="reason" placeholder="Enter rejection reason ..." ></textarea></div>';
                //hod only approval
                 }else{
                   if (data[i]['hod_reason'] != null) {
                    ...
                    result1+= '<div class="row"><div class="col-md-5"><b>HOD Status</b></div><div class="col-md-5">:  '+data[i]['hod_status']+'</div></div>';
                    result1+= '<div class="row"><div class="col-md-5"><b>HOD responded date</b></div><div class="col-md-5">:  '+data[i]['hod_created_date']+'</div></div>';
                    result1+= '<div class="row"><div class="col-md-5"><b>Admin Status</b></div><div class="col-md-5">:  '+data[i]['admin_status']+'</div></div>';
                    result1+= '<div class="row"><div class="col-md-5"><b>Admin responded date</b></div><div class="col-md-5">:  '+data[i]['admin_created_date']+'</div></div>';
                    result1+= '<div class="row"><div class="col-md-5"><b>Final Status</b></div><div class="col-md-5">:  '+data[i]['final_status']+'</div></div>';
                 }

        ...
        <div class="container">
          <div class="row">
            <div class="col-sm-12">
              ...
              <?php $projectname_str = strtolower(str_replace(' ', '_', $completed->Project_name)); ?>
              <!-- Single Portfolio-->
                <li class="col-sm-6 col-md-4 single-port-item {{$completed->Type}}-1">
              ...
              <div class="card marginrightzero">
                <div class="card-image">
                  <div class="card-img-wrap">
                    ...
                    @if($completed->no_details != null)
                      <a href="#">
                    @else
                    <a href="{{ url('/project')}}/{{ $projectname_str }}">
                    @endif
                      @if(file_exists( public_path() . '/images/project_thumb/' . $completed->id . '.jpg'))

                        <img class="activator img-responsive" src="/images/project_thumb/{{$completed->id}}.jpg" alt="{{$completed->Project_name}}">
                      @else
                          @if($completed->Type == 'Apartments')
                          <img class="activator img-responsive" src="/images/project_thumb/default_flats.jpg" alt="{{$completed->Project_name}}">
                          @elseif($completed->Type == 'Plots')
                          <img class="activator img-responsive" src="/images/project_thumb/default_plots.jpg" alt="{{$completed->Project_name}}">
                          @else
                          @endif
                      @endif
                    ...
                    <div class="post-body">
                    @if($completed->no_details != null)
                      <a href="#" class="post-title-link brand-text"><h2 class="post-title">VGN {{$completed->Project_name }}</h2></a>
                    @else
                    <a href="{{ url('/project')}}/{{ $projectname_str }}" class="post-title-link brand-text"><h2 class="post-title">VGN {{$completed->Project_name }}</h2></a>
                    @endif
                      @if($completed->Location != null)
                      <p class="post-content">Location: {{ $completed->Location }}</p>
                      @endif
                      @if($completed->Type != null)
                      <p class="post-content">Project Type: {{ $completed->Type }}</p>
                      @endif
                    </div>
                  </div>
                </div>

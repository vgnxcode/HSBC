...
@section('title')
VGN Projects Estates Pvt Ltd |Vendor Zone| Login Page
@endsection

@section('description')
     <META NAME="Subject" CONTENT="Flats and apartments in Chennai">
<meta name="description" content="VGN Vendor Login">
...
<meta property='og:url' content='http://vgn.in/vendorzone/vendorlogin'/>
...
<div class="login-box">
 <a href="{{ url('/')}}">
  ...
  <div class="login-box-body box-body">
    <h4 class="login-box-msg" style="background-color:#f7f7f7; font-size: 18px; text-align: center; padding: 7px 10px; margin-top: 0;">Vendor Login</h4>
    <br>  
    ...
    <form id="signinform" method="POST" action="{{ url('/vendorzone/vendorlogin')}}">
    @csrf
      <div class="form-group has-feedback">
        <span class="glyphicon glyphicon-user form-control-feedback"></span>
        <input type="text" class="form-control" name="username" id="cid"  value="{{ old('username')}}" placeholder="Enter Vendor ID (or) Regd. Mail ID (or) Mobile">
...
<form id="forgotform" action="">
      <div class="form-group has-feedback">
        <span class="glyphicon glyphicon-user form-control-feedback"></span>
        <input type="text" class="form-control" id="forgotusername" name="forgotusername" id="fcid"  value="" placeholder="Enter Vendor ID (or) Regd. Mail ID (or) Mobile">
  ...
  <div class="row" id="votpresponsetext" style="margin: 2px;">
    ...
    <a href="#" id="vlogin"><i class="fa fa-user margin-r-5"></i> Vendor Login</a><br>  
...
<script>
  ...
  $("document").ready(function() {
    ...
    $("#vlogin").on("click", function(){
      ...
      $("#vlogin").removeAttr("style").hide();
      $("#spinner").show();
      $(".login-box-msg").text('Vendor Login');
      setTimeout(function(){
        $("#signinform").show();
    ...
    $("#sendotp").on("click", function(){
      ...
      $("#spinner").show();
      if (username != '') {
        $.post("{{ url('/vendorzone/forgotpassword')}}",{_token:'{{csrf_token()}}','forgotusername' : username}, function(data){
          $("#spinner").hide();
          console.log(data);
          if (data == '1') {
            $("#sendotp").text('Resend OTP');
            $("#sendotp").attr('disabled',true);
            $("#votpresponsetext").html(`<div clas="row" id="otpdiv"><p style="margin-top:5px; font-weight:500;">Please enter the OTP Sent to your registered mobile number and Mail ID. OTP expires in <span id="otpexpirycounter" style="font-weight:bold;">90</span> Seconds.</p>
              <br>
      ...
      <div class="row">
        ...
        <div class="col-xs-6">
          <button type="button" onclick="vsubmitotpfn();" class="btn btn-danger btn-block btn-flat">Submit OTP</button>
        </div>        
      </div>
              </div>`);
            CCOUNT  = '90';
            ...
            alert(data);
            window.location.href="/vendorzone/vendorlogin";
          }
          //$("#otpsubmit").show();
        });  
      }
  ...
  if ((username1 != '') && (submitotp != '')) {
    $.post("{{ url('/vendorzone/otpvalidation')}}",{_token:'{{csrf_token()}}','forgotusername' : username1,'submitotp': submitotp}, function(data){
      $("#spinner").hide();
        if(data == '1'){
            window.location.href='/vendorzone/resetlink';
        }
        else{
